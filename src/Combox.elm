...
view model (Config config_) =
    Html.div config_.options
        [ titlewrapper config_.title
        , Selectize.view
            (viewAutocomplete (Config config_))
            model.selection
            model.menu
            |> Html.map MenuMsg
        ]
        |> Html.map config_.toMsg
...
viewAutocomplete (Config config_) =
    viewConfig <|
        Selectize.autocomplete <|
            { attrs =
                \sthSelected open ->
                    ...
                    , Attributes.classList
                        [ ( "selectize__textfield--selection c-black", sthSelected && not config_.disabled )
                        , ( "selectize__textfield--no-selection c-gray", not sthSelected || config_.disabled )
                        , ( "selectize__textfield--menu-open", open )
                        , ( "selectize__textfield--menu-disabled", config_.disabled )
                        ]
            ...
            , clearButton =
                case not config_.disabled && config_.clear of
                    True ->
                        clearButton

                    False ->
                        Nothing
            , placeholder = Maybe.withDefault "All" config_.placeholder
            }
